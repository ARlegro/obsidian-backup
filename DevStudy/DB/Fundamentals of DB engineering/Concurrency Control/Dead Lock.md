> 여러 트랜잭션이 서로의 자원을 기다리면서 영원히 진행되지 못하는 상태

목차
- Dead락 정의
- 발생 조건 (4자지)
- Deadlock 발생 사례
- PostgreSQL의 DeadLock 감지 및 해결 방식 
- OS수준에서의 Dead Lock 대응 전략 
- Java 코드에서의 Dead Lock 발생과 해결 


## 1. 데드락이란❓ (교착 상태)

> 두 개 이상의 트랜잭션이 **서로 다른 자원(예: 테이블의 행, 인덱스 엔트리 등)을 점유한 상태에서 상대방이 점유한 자원을 요청하며 무한정 기다리는 교착 상태**를 의미
- 주로 동시성 환경에서 발생하낟.
- DB, OS, 멀티스레딩 서버 환경에서 모두 발생 가능하다


## 2. Dead Lock 발생 4조건 
> [!EXAMPLE] Dead Lock은 아래의 4가지 조건을 모두! 만족될 때만 일어난다.
> - **Mutual Exclusive (상호 배제)**
> 	- 리소스를 공유해서 사용할 수 없다.
> 	- 즉, 리소스는 한 번에 하나의 프로세스만 사용이 가능 
> - **Hold and Wait (점유 대기)**
> 	- 이미 리소스를 점유한 상태에서 **추가 자원을 요청하며 대기** 
> 	- 다른 프로세스에 할당된 자원을 점유하기 위해 대기하는 프로세스가 되어버림 
> - **No preemption(비선점)**
> 	- 리소스 반환(release)은 오직 그 리소스를 취득한 프로세스만 가능하다.
> 	- 즉, **다른 프로세스가 사용 중인 자원을 강제로 뺏을 수 없음**
> - **Circular wait(순환대기)**
> 	- 프로세스들이 순환 형태로 서로의 리소스를 기다린다.

![[Pasted image 20250605133547.png]]
특히나 DB를 여러개 사용할 경우 위의 사진처럼 ROCK이 일어나면 엄청난 network 비용이 든다.

> 4가지 중 단 하나라도 깨지면 DeadLock은 발생하지 않음 

## 3. 데드락이 발생 예시 - PostgreSQL 트랜잭션

#### 트랜잭션 흐름 예 
두 트랜잭션 `T1`과 `T2`가 `A`라는 행과 `B`라는 행에 접근한다고 가정

| 시간  | 트랜잭션 T1             | 트랜잭션 T2             |
| :-- | :------------------ | :------------------ |
| 1   | `LOCK A` (A를 잠금)    |                     |
| 2   |                     | `LOCK B` (B를 잠금)    |
| 3   | `LOCK B` (B를 잠금 시도) |                     |
| 4   |                     | `LOCK A` (A를 잠금 시도) |

- 상황 : `T1`은 `A`를 잠근 후 `B`를 잠그려 하고, `T2`는 `B`를 잠근 후 `A`를 잠그려 하는 상황
- 대기
	- 시간 3에서 `T1`은 `B`가 `T2`에 의해 잠겨 있어 대기합니다.
	- 시간 4에서 `T2`는 `A`가 `T1`에 의해 잠겨 있어 대기합니다.
- 결과적으로
	- `T1`은 `T2`가 `B`를 놓아주기를 기다리고, `T2`는 `T1`이 `A`를 놓아주기를 기다리며, 
	- 둘 다 영원히 진행되지 못하는데, 이것이 "**데드락**"이다.

>정리 : T1, T2 서로의 자원을 기다리며 순환 대기 상태 진입 ➡ Deadlock 발생

#### 실험 전 초기 상태
```bash
        List of relations
 Schema | Name | Type  |  Owner   
--------+------+-------+----------
 public | test | table | postgres
(1 row)

postgres=# select * from test;
 id 
----
(0 rows)
```
- 테이블만 존재하고 내용은 비어 있다.
#### 테스트 : 트랜잭션 흐름 및 데드락 발생 
```SQL
T1
BEGIN transaction;
INSERT INTO test VALUES(20); -- 아직 COMMIT하지 않음

T2
BEGIN transaction;
INSERT INTO test VALUES(21); -- 정상 실행
INSERT INTO test VALUES(20);
-- ❗대기 상태(락) 발생 (T1이 같은 값 20을 먼저 넣었기 때문에)

T1 
INSERT INTO test VALUES(20);
❌ 데드락 오류 발생
```

```sql
**🔐발생 오류** 
ERROR:  deadlock detected
DETAIL:  Process 72 waits for ShareLock on transaction 757; blocked by process 145.
Process 145 waits for ShareLock on transaction 758; blocked by process 72.
HINT:  See server log for query details.
CONTEXT:  while inserting index tuple (0,4) in relation "test_pkey"
```

**❓왜 데드락이 발생했는가❓**
1. T1이 먼저 `id=20`을 INSERT함 
	- 아직 커밋하지 않았지만 **MVCC에 의해 메모리에는 존재함**
2. 이후 T2가 동일한 `id=20`을 INSERT 하려 함 
	- PK 제약 조건 위반 여부 확인 중, T1이 커밋을 안 했으므로 **T2는 T1의 트랜잭션 결과를 기다림**
	  
3. 그러다 T1이 다시 **같은 값을 INSERT 시도** 
	- 이 시점에서 **T1은 T2의 트랜잭션을 기다리게 됨**

- T1은 T2의 락을 기다림
- T2는 T1의 락을 기다림  
    → 🔁 **데드락 조건(Circular Wait)이** 충족되며 데드락 발생
![[Pasted image 20250605132504.png]]
## 4. PoswgreSQL의 Deadlock 감지 및 해결 

**⛔감지 매커니즘** 
- 락 대기 그래프를 지속적으로 감지 
	- PostgreSQL은 **락 대기 그래프**를 주기적으로 확인하고 **사이클이 발견되면 데드락으로 판단**
		- 트랜잭션 간 의존관계를 그래프의 형태로 추적
	- 그래프에 Cycle이 발생하면 Deadlock 판점 

**✅해결 방식** 
- Cycle을 끊기 위해 **가장 늦게 트랜잭션에 진입한 프로세스**를 **자동 롤백**시켜 데드락 해소

> Postgres의 데드락 해결법 ➡ 데드락 일으킨 트랜잭션 **롤백** 시키기 

#### Deep - 커밋하지 않았는데 왜 Lock ❓
>[!question] 아무것도 커밋하지 않았는데 Lock이 일어난 이유 ❓
>- 트랜잭션은 커밋되지 않았지만, MVCC에 의해 **메모리에는 이미 반영**
>- PostgreSQL은 **메모리상의 Row도 Lock 체크에 포함** 시키기 때문에 커밋하지 않아도 데드락을 감지할 수 있는 것 


## 5. OS 수준에서의 Deadlock 해결 방법 
>[!tip] 아래의 방법들이 있다. (하지만 속 시원한 해결은 없다)
>- 데드락 방지
>- 데드락 회피
>- 데드락 감지와 복구
>- 데드락 무시 

>[!tip] 실제 시스템에서는 완전한 Deadlock 방지보다는, **감지와 회피를 병행**하는 방식이 일반적  
>- 예측 가능한 순서대로 자원을 요청하고  
>- Deadlock 발생 시 일부 트랜잭션을 롤백하는 방식이 현실적인 접근

### 데드락 방지 
앞서 말한 데드락 발생 조건 중 하나가 충족되지 않게 **시스템을 디자인하는 것.**
`4가지 조건을 만족해야 데드락이 된다는 것 = 하나라도 만족하지 않으면 데드락 실패`

#### 방법 1. Mutual Exclusive(상호 배제) 조건 방지 

**✔상호배제 개념** 
- 리소스는 오직 **하나의 프로세스만 독점 사용** 가능할 때 해당 조건이 성립됨

**✅방지 방법** 
- 리소스를 **여러 프로세스가 동시에 공유 가능하도록 설계**
- ex. 읽기 전용 데이터는 여러 트랜잭션이 동시에 읽도록 허용 (Read lock만)

**💢실무적 한계** 
- **비현실적** : 현실적으로는 불가능 Cuz 공유를 막는 이유가 있겠죠 
	- e.x 쓰기 작업, 디스크 블록, 테이블 행 등은 동시 접근 시 데이터 무결성 훼손 위험
- 따라서 이 조건은 **실제 시스템에서는 거의 반드시 성립**되며, 깨뜨리기 어려움

#### 방법 2. Hold and Wait(점유 대기) 조건 방지 

**✔점유 대기 개념** 
- 리소스를 이미 하나 이상 점유한 상태에서 **추가로 리소스를 요청하며 대기**

**✅방지방법** 
- **필요한 모든 리소스를 한 번에 요청** → 모두 획득되면 실행 시작
- 또는, **리소스를 점유하지 않은 상태에서만 요청 가능**하게 제한

>이렇게 하면 대기할 필요도 없고, 리소스를 점유하고 있지 않아서 그 리소스를 필요로하는 다른 프로세스(스레드)에도 영향 X 

**💢실무 한계** 
- 리소스를 미리 다 확보하면 **자원 활용률 저하**
    - 어떤 리소스는 나중에 필요할 수도 있는데, 미리 잠금
    - 사용하지도 않는 자원을 오랫동안 보유
- **대기 시간 증가 및 병목 발생**

#### 방법 3. No preemption(비 선점) 조건 방지 

**✔비 선점 개념**
- 점유한 리소스를 **강제로 회수할 수 없는 경우**

**✅방지 방법** 
- 자원을 기다리는 프로세스가 있다면, **이미 점유한 리소스를 반납하게 강제**
	- EX. 트랜잭션이 필요한 리소스를 얻지 못하면 **지금까지의 리소스 반납 + 재시도**
- 즉, 이미 선점해도 다른 프로세스에게 **양보**하도록 함

**⛔실무 한계** 
- 강제 회수는 **복잡한 상태 관리**가 필요함 (ex. 중간처리 작업도 되돌리는 로직)
- 이로 인해, 일관성 보장이 어려워지고 시스템 설계가 어려워짐 
	  

#### 방법 4. Circular Wait(순환 대기) 조건 방지 ⭐

**✔순환 대기 개념** 
- 여러 프로세스가 서로가 가진 리소스를 기다리며 순환 형태의 고리가 형성 

**✅방지 방법 (가장 널리 사용됨)**
- **모든 리소스에 고정된 우선순위(번호)를 부여**    
- 각 프로세스는 **항상 번호가 높은 리소스만 요청하도록 강제**
    - 예: 파일 → DB → 네트워크 순서대로만 요청 가능

> 데드락 방지 기술 중 많이 쓰이는 방식 ⭐

**⛔실무 한계**
- 우선순위 고정으로 인한 **유연성 저하** 
	- 요청 순서의 변경이 필요할 경우가 문제다.
	- 이럴 경우 우선순위에 맞지 않게 동작하려면 전체 재설계 필요


### 데드락 회피 
데드락 방지 = 시스템 디자인을 통해 방지 

>[!tip] 실행 환경에서 추가적인 정보를 활용하여 데드락이 발생할 것 같은 상황을 회피하는 것

대표적인 데드락 회피 알고리즘 = **Banker Algorithm**
- 리소스 요청을 허락해줬을 때, 데드락이 발생할 가능성이 있다면 리소스를 할당해도 **안전할 때까지 계속 요청을 거절** 하는 알고리즘 
### 데드락 감지와 복구 

>[!tip] 데드락을 **허용**하고 데드락이 **발생하면 복구**하는 전략 

데드락 감지 후 **복구 전략은 크게 2가지**이다.
1. **프로세스 종료**
	- 극단적이긴하다. 
	- 그러나 현업에서 가끔씩 쓰기도 한다.
	  
2. **리소스의 일시적 선점을 허용하는 것** 


#### 데드락 무시 

>은근히 OS가 이런 방식을 많이 쓴다.


